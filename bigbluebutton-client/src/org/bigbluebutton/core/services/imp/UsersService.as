...
package org.bigbluebutton.core.services.imp {
	...
	public class UsersService extends Actor {
        [Inject]
	    ...
	    public function listenForUserMessages():void {
            logger.debug("invoking listen for user messages");
        ...
        private function parseUser(joinedUser:Object):User { 
            ...
            logger.debug("User joined " + user.toString());
            return user;       
        }
...
*/		}
        ...
        private function syncEventHandler(event:SyncEvent):void {
            ...
            logger.debug("SYNC_EVENT: " + statusCode);
            logger.debug(event.changeList.length.toString());
            if (event.changeList.length > 0) {
                for (var i:int = 0; i < event.changeList.length; i++ ) {
                    logger.debug("Event change: " + event.changeList[i].code);
                }                
            }
            dispatch(new ListeningForUserMessagesEvent());
        }
        
		private function netStatusHandler(event:NetStatusEvent):void {
			var statusCode:String = event.info.code;
			logger.debug("*** SO " + statusCode);
            
			switch (statusCode)  {
				case "NetConnection.Connect.Success" :
					logger.debug("Listening for user messages ");		
					dispatch(new ListeningForUserMessagesEvent());			
					break;
			
				case "NetConnection.Connect.Failed" :			
                    logger.debug(":Connection to viewers application failed");
				...
				case "NetConnection.Connect.Rejected" :
                    logger.debug(":No permissions to connect to the viewers application" );
					sendConnectionFailedEvent(ConnectionFailedEvent.CONNECTION_REJECTED);
					break;
					
				default :
                    logger.debug(":default - " + event.info.code );
		...
		{
			logger.debug("participantsSO asyncErrorHandler " + event.error);
